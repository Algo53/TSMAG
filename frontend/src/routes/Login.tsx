...
function Login() {
  ...
  return (
    <div className='flex w-full h-full justify-center items-center md:px-0 sm:px-10 px-3'>
      <div className='flex h-2/3 xl:w-1/2 lg:w-3/4 md:w-10/12 w-full shadow-xl hover:shadow-2xl rounded-3xl'>
        <div className='xs:flex hidden xs:w-1/2 h-full'>
          <img className='w-full h-full rounded-l-3xl' src='/images/loginPage.jpg' alt='' />
        </div>
        <div className='flex xs:w-1/2 w-full h-full justify-center xs:shadow-inner xs:rounded-r-3xl'>
          <div className='flex flex-col w-full h-full lg:px-7 md:px-5 sm:px-2 xs:px-1 pt-3 items-center gap-8'>
            ...
            <button className={`flex xs:w-full px-16 xs:px-0 justify-center text-xl ${isFormValid? "hover:text-[19px] hover:shadow-inner hover:bg-zinc-200" : ""} cursor-pointer bg-zinc-300 py-2 rounded-3xl`} onClick={handleSubmit} disabled={!isFormValid}>Login</button>
