...
const loginUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const hashedPassword = (0, bcryptHelper_1.comparePassword)(password, user.password);
        if (!bcryptHelper_1.hashPassword) {
            res.status(400).json({ success: false, message: 'Invalid Credentials!' });
            return;
        }
        // Generate the jwt token and send it to the client
        const token = (0, jwtHelper_1.accessToken)(user._id);
        // Send a success response
        res.status(200).json({
            success: true,
            accessToken: token,
